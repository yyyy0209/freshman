...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"12px 10px 20px","margin":"20px 0 0 0","borderColor":"#52b646","alignItems":"center","borderRadius":"12px","flexWrap":"wrap","background":"#eff9ee","borderWidth":"4px","display":"flex","width":"100%","borderStyle":"solid dashed dotted double","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
        ...
        <el-select v-model="formSearch.xueshengxingming" placeholder="请选择学生姓名" :clearable="true">
          <el-option v-for="(item, index) in xueshengxingmingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/xuesheng/xueshengxingming').then(res => {
        if (res.data.code == 0) {
          this.xueshengxingmingOptions = res.data.data;
        }
      });
	    this.jiaofeileixingOptions = '住宿费,学费,其他费用'.split(',');
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
