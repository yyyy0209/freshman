...
<div :style='{"padding":"20px 20px 40px","margin":"10px auto 0","borderColor":"#52b646","borderRadius":"12px","background":"#fff","borderWidth":"3px","width":"1200px","position":"relative","borderStyle":"solid dashed double double"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="报道标题" prop="baodaobiaoti">
            <el-input v-model="ruleForm.baodaobiaoti" 
                placeholder="报道标题" clearable ></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="流程内容" prop="liuchengneirong">
            <editor 
  ...
  export default {
    data() {
      return {
        ...
        rules: {
          baodaobiaoti: [
            { required: true, message: '报道标题不能为空', trigger: 'blur' },
          ],
          fengmian: [
          ],
          liuchengneirong: [
    ...
    methods: {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            if(o=='baodaobiaoti'){
              this.ruleForm.baodaobiaoti = obj[o];
              this.ro.baodaobiaoti = true;
              continue;
            }
            if(o=='fengmian'){
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('baodaoliucheng/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('baodaoliucheng/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('baodaoliucheng/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
