...
<div :style='{"padding":"20px 20px 40px","margin":"10px auto 0","borderColor":"#52b646","borderRadius":"12px","background":"#fff","borderWidth":"3px","width":"1200px","position":"relative","borderStyle":"solid dashed double double"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="系别" prop="xibie">
            <el-input v-model="ruleForm.xibie" 
                placeholder="系别" clearable ></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="分配时间" prop="fenpeishijian" >
              <el-date-picker
...
<script>
  export default {
        ...
        this.$http.get('follow/xuesheng/xuehao?columnValue=' + this.ruleForm.xuehao, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if(res.data.data.shoujihaoma){
              this.ruleForm.shoujihaoma = res.data.data.shoujihaoma
            }
            if(res.data.data.xibie){
              this.ruleForm.xibie = res.data.data.xibie
            }
            if(res.data.data.zhuanye){
              this.ruleForm.zhuanye = res.data.data.zhuanye
            }
          }
        });
      },
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('banjifenpei/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('banjifenpei/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('banjifenpei/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
